...
describe('LoginPage', () => {
  ...
  it('HU-02 rechaza credenciales incorrectas mostrando el mensaje del backend', async () => {
    mockJsonResponse({ error: 'Correo o contrasena incorrectos. Intento 2 de 5.' }, false, 401);
    renderLogin();
    fillAndSubmit();

    expect(await screen.findByText('Correo o contrasena incorrectos. Intento 2 de 5.')).toBeInTheDocument();
  });
