      ...
      if (error) {
        ...
        return {
          ...
          error: {
            ...
            message: isDuplicate
              ? 'Este correo ya esta registrado. Por favor inicia sesion o usa otro correo.'
              : error.message,
          },
        };
      }
    ...
    async login(email, password) {
      ...
      if (error || !data.session || !data.user) {
        ...
        return {
          ...
          error: {
            status: 401,
            message: `Correo o contrasena incorrectos. Intento ${attempt?.intentos ?? 1} de 5.`,
          },
        };
      }
    ...
    async create(input) {
      ...
      if (error) {
        ...
        return {
          ...
          error: {
            status: isDuplicate ? 409 : 400,
            message: isDuplicate ? 'Ya existe un medico con esta cedula profesional.' : error.message,
          },
        };
      }
