...
export function createInMemoryServices() {
  ...
  const especialidades: Especialidad[] = [
    { id: 'esp-1', nombre: 'Cardiologia' },
    { id: 'esp-2', nombre: 'Pediatria' },
    { id: 'esp-3', nombre: 'Dermatologia' },
  ];
  ...
  const auth: AuthService = {
    ...
    async login(email, password) {
      ...
      if (!user) {
        ...
        return {
          ok: false,
          error: { status: 401, message: `Correo o contrasena incorrectos. Intento ${current.intentos} de 5.` },
        };
      }
    ...
    async forgotPassword() {
      // En memoria no hace nada; la implementacion real llama a Supabase Auth.
    },
  ...
  const medicosService: MedicosService = {
    ...
    async create(input) {
      if (medicos.some((m) => m.licencia === input.licencia)) {
        return { ok: false, error: { status: 409, message: 'Ya existe un medico con esta cedula profesional.' } };
      }
