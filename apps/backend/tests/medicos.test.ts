...
describe('GET /api/especialidades', () => {
  it('HU-04 lista especialidades disponibles', async () => {
...
describe('POST /api/medicos', () => {
  it('HU-04 registra un medico con especialidad cuando el usuario es ADMIN', async () => {
    ...
    expect(response.body.message).toBe('Medico registrado correctamente.');
  });
  ...
  it('HU-04 bloquea el registro con licencia duplicada', async () => {
    ...
    expect(response.body.error).toBe('Ya existe un medico con esta cedula profesional.');
  });
  ...
  it('HU-04 rechaza el registro si el usuario no es ADMIN', async () => {
    ...
    expect(response.body.error).toBe('No tienes permisos para acceder a esta seccion.');
  });
});
